...
```{r multiModel2latex, echo=FALSE}
stargazer(finalModel, type = 'latex', p.auto = FALSE, out="../Data/multi.tex", single.row = TRUE) # tabulating the results
```
...
leaflet(height=500) %>% # initializing the leaflet map
  ...
  addPolygons(data = cty_sf, stroke = FALSE, fillColor = ~myPal(cty_sf_joined$match), # adding the data
              weight = 2, opacity = 1, color = "white", dashArray = "3", fillOpacity = 0.7, # adding color specs
              popup = paste("County:", cty_sf_joined$NAME_2, '<br>', 
...
```{r staticMapResults}
d3map2 = d3_map(shape = cty_sf_joined, projection = "Albers", height = 500) %>%
