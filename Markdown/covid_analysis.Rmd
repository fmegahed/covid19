...
knitr::include_graphics("../Figures/cdcRegionsV1.png")
```
...
In this subsection, we provide seven sets of plots:
  ...
  - A panel of 9 sample scaled time-series for each cluster;  
  - A paneled spaghetti plot, highlighting the median scaled time-series for profile for each cluster;
  - A panel plot where the first, second and third quartiles of the scaled time-series for each cluster are compared;  
...
### Samples of Scaled Time-Series Per Cluster {- .tabset .tabset-fade .tabset-pills}

```{r clusterSamples, results='asis'}
...
ggsave('../Figures/spaghettiPlot.png', width = 7.5, height = 3.75, units = 'in', dpi = 320)

```

### Summaries of the Scaled Time-Series for Each Cluster {-}
...
dfAvg %<>% pivot_longer(cols = c(`First Quartile`, Median, `Third Quartile`),
                        names_to = 'Statistic') 
...
d3map2 = d3_map(shape = cty_sf_joined, projection = "Albers", height = 768) %>%
...
# Using the save_d3_png() function requires that you install the webshot package, as well as the
# phantom.js headless browser (which you can install using the function webshot::install_phantomjs()).
r2d3::save_d3_png(d3map2, file = '../Figures/CountyMatches.png', width = 1366, height = 768, zoom = 2)
