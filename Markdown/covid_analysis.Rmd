...
knitr::include_graphics("../Figures/SampleCounties02.png")
```

### A Sample of New Cases {-}
```{r newDailyCasesSample, echo=FALSE,  out.width = '100%'}
...
df$cluster_group %<>%  recode(C2 = '1st.Wave', C1 = '2nd.Wave', C3 = 'Other') %>% # recoding values
  factor(levels = c('1st.Wave', '2nd.Wave', 'Other'))
# setting the reference level to category with max frequency
df$clustReLeveled = relevel(df$cluster_group, ref = maxCat(df$cluster_group) ) #(w/ data ending in 2020-10-03 --> = `no wave` group i.e. C3)
...
mapResults$cluster_group %<>%  recode(C1 = '2nd.Wave', C2 = '1st.Wave', C3 = 'Other') %>% # making names more informative
  factor(levels = c('1st.Wave', '2nd.Wave', 'Other')) # making the levels appear in the order of outbreak
...
mapResults$cluster_group %<>% factor(levels = c('1st.Wave', '2nd.Wave', 'Other')) # making the levels appear in the order of outbreak
...
mapResults$cluster_group %<>% factor(levels = c('1st.Wave', '2nd.Wave', 'Other')) 
...
- On a county-level, the time series of COVID-19 new daily cases can be clustered into one of three clusters: (a) `Cluster 3 (baseline)` contains 1,476 counties, which have had a limited outbreak of COVID-19, (b) `Cluster 1` contains 1,214 counties which have primarily experienced a second wave of COVID-19, and (c) `Cluster 2` contains 418 counties which have predominately experience a first wave of COVID-19. Note that our total number of counties are slightly less than the number of counties in the contiguous U.S. since we did not have complete data on the confirmed cases for those counties.  
