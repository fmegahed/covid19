...
pacman::p_load(tidyverse, magrittr, dataPreparation, recipes, doParallel, psych, skimr, VIM, reshape2, # for data analysis
               ...
               leaflet, choroplethr, choroplethrMaps, # for mapping
...
In this section, we utilize the [COVID19 package](https://cran.r-project.org/web/packages/COVID19/COVID19.pdf) to obtain the following information: [@Guidotti2020]    
  ...
  - policy information (e.g., transport closing, school closing, closing event, movement restrictions, testing policies, and contact tracing);  
...
hospitals = read.csv("../Data/data-FPBfZ.csv") %>% # downloaded from KHN on 2020-09-01 (9:55 am EDT)
...
regionsCDC = data.frame(States = c('Connecticut', 'Maine', 'Massachusetts', 'New Hampshire', 'Rhode Island' , 
                                   ...
                                   'Arizona', 'California', 'Hawai', 'Nevada', # Region I
...
nc = NbClust(clusteringPrep, distance = "euclidean", # euclidean distance
             ...
             index = "all") # using 26 of the 30 indices in the package (we left the 4 that were computationally intensive)
...
In this subsection, we have created two chloropleth maps to visualize the clusters. The first map provides some interactivity features, where the reader can click on a given county to show: (a) county name, (b) assigned cluster, (c) population density, and (d) percentage of residents in poverty. In the second map, we provide a static version of the same choropleth (i.e., only showing the cluster assignment for a given county). Both maps can be accessed by toggling through the tabsets below.
...
LeafletCounties = clusterCounties %>% ungroup() %>%
  dplyr::select(administrative_area_level_2, administrative_area_level_3, cluster_group, popDensity, povertyPercent)
colnames(LeafletCounties) = c('NAME_1', 'NAME_2', 'value', 'popDensity', 'povertyPercent')

USA = getData(country = "usa", level = 2) # Get US polygon data from raster
LeafletCounties = merge(USA, LeafletCounties, all.x = TRUE) # merging them
...
leaflet(height=500) %>% # initializing the leaflet map
  ...
  addPolygons(data = USA, stroke = FALSE, fillColor = ~myPal(LeafletCounties$value), # adding the data
...
```{r staticCluster}
# ungrouping clusterCounties and selecting just the two variables needed for the clustering
vizCounties = clusterCounties %>% ungroup() %>% select(key_numeric, cluster_group)

colnames(vizCounties) = c('region', 'value') #renaming columns to match required inputs for county_choropleth()
levels(vizCounties$value) = paste0('Cluster #', levels(vizCounties$value) ) # Prefix Cluster # to each cluster number 

# Creating the Map Using ggplot options
choro = CountyChoropleth$new(vizCounties) #creating the plot object
choro$ggplot_scale = scale_fill_brewer(name="Cluster", type = "qual", palette= 'Set2') # using colorbrewer colors
choro$render() + theme(legend.position = "bottom") # rendering the plot
```
...
df$cluster_group %<>%  recode(C1 = '2nd.Wave', C2 = '1st.Wave', C3 = 'Other') # recoding values
# setting the reference level to category with max frequency
df$clustReLeveled = relevel(df$cluster_group, ref = maxCat(df$cluster_group) ) #(w/ data ending in 2020-09-19 --> = `no wave` group i.e. C3)
...
#### Logit Coefficients and their Significance {-}
```{r multiModel2, results='asis'}
stargazer(finalModel, type = 'html', p.auto = FALSE, out="../multiNomFinal/multi.htm") # tabulating Final Model 
```
...
```{r multiSummary}
summary(finalModel) # print a summary of the model 
```

#### z-Score {-}
```{r zScore, results='asis', out.width='50%'}
z = summary(finalModel)$coefficients / summary(finalModel)$standard.errors # computing the z-score
round(z, 3) %>% as.data.frame() %>% rmarkdown::paged_table() # printing it nicely  
```
...
```{r multiVizDataPrep}
predictedProbs = fitted(finalModel) # computing predicted probabilities for each of three cluster outcome levels

multiClassResults = cbind(na.omit(multiClassData), predictedProbs) %>%  # column binding the predicted probs to multiClassData
...
multiClassCat$LargestProbCluster = colnames(multiClassCat[, 9:11])[apply(multiClassCat[, 9:11], 1, which.max)] 
...
colorPal =  brewer.pal(n=3, 'Set2')
names(colorPal) = levels(multiClassData$cluster_group) %>% 
  recode_factor(C1 = '2nd.Wave', C2 = '1st.Wave', C3 = 'Other')
...
```{r probVspopDensity}
ggplot(resSummary, aes(x = popDensity, y = probability, color = variable)) + # setting x-axis, y-axis and color 
...
```{r probVsRep}
ggplot(resSummary, aes(x = percRepVotes, y = probability, color = variable)) + # setting x-axis, y-axis and color 
  geom_line() + facet_grid(variable ~ .) +
  scale_color_manual(name = "Predicted Cluster", values = colorPal) + 
  scale_y_continuous(limits = c(0,1)) + # color and y-axis limits
...
```{r probVsSeniors}
ggplot(resSummary, aes(x = PercentSeniors, y = probability, color = variable)) + # setting x-axis, y-axis and color 
  geom_line() + facet_grid(variable ~ .) +
  scale_color_manual(name = "Predicted Cluster", values = colorPal) + 
  scale_y_continuous(limits = c(0,1)) + # color and y-axis limits
...
```{r probVsicuBeds}
ggplot(resSummary, aes(x = icuBedsPer10000Residents, y = probability, color = variable)) + # setting x-axis, y-axis and color 
  geom_line() + facet_grid(variable ~ .) +
  scale_color_manual(name = "Predicted Cluster", values = colorPal) + 
  scale_y_continuous(limits = c(0,1)) + # color and y-axis limits
...
```{r probVsPoverty}
ggplot(resSummary, aes(x = povertyPercent, y = probability, color = variable)) + # setting x-axis, y-axis and color 
  geom_line() + facet_grid(variable ~ .) +
  scale_color_manual(name = "Predicted Cluster", values = colorPal) + 
  scale_y_continuous(limits = c(0,1)) + # color and y-axis limits
...
```{r, clusterByRegions}
multiClassCat %>% select(c(LargestProbCluster, regions)) %>% # selecting two columns of interest
  group_by(LargestProbCluster, regions) %>% count() %>% # counting occurrences of combinations of those columns
...
```{r, clusterByGovernor}
multiClassCat %>% select(c(LargestProbCluster, governorsParty)) %>% # selecting two columns of interest
  group_by(LargestProbCluster, governorsParty) %>% count() %>% # counting occurrences of combinations of those columns
...
multiClassCat %>% select(c(LargestProbCluster, countyType)) %>% # selecting two columns of interest
  group_by(LargestProbCluster, countyType) %>% count() %>% # counting occurrences of combinations of those columns
...
mapResults$cluster_group %<>%  recode(C1 = '2nd.Wave', C2 = '1st.Wave', C3 = 'Other') # making names more informative

# For each county, we find the cluster with largest prob from the multinomial model's results
mapResults$LargestProbCluster =  colnames(mapResults[, 14:16])[apply(mapResults[, 14:16], 1, which.max)] 

# Does the cluster name with largest prob match the one assigned from the k-means clustering approach?
mapResults$match = ifelse(mapResults$cluster_group == mapResults$LargestProbCluster, 'Yes', 'No') %>% as.factor()

# Merging with the USA data frame from the previous leaflet plot
mapResults = merge(USA, mapResults, 
                   by.x = c('NAME_1', 'NAME_2'), 
                   by.y = c('administrative_area_level_2', 'administrative_area_level_3'),
                   all.x = TRUE) # merging them

# using same color scheme as the static one
myPal = colorFactor('Paired', domain = mapResults$match, na.color = "white")
  
leaflet(height=500) %>% # initializing the leaflet map
  setView(lng = -96, lat = 37.8, zoom = 3.8) %>% # setting the view on Continental US
  addTiles() %>% # adding the default tiles
  addPolygons(data = USA, stroke = FALSE, fillColor = ~myPal(mapResults$match), # adding the data
              weight = 2, opacity = 1, color = "white", dashArray = "3", fillOpacity = 0.7, # adding color specs
              popup = paste("County:", mapResults$NAME_2, '<br>', 
                            "Cluster #:", mapResults$cluster_group, '<br>',
                            'Cluster Pred from Multinomial:', mapResults$LargestProbCluster, '<br>', 
                            "Population Density:", round(mapResults$popDensity, 1), '<br>',
                            "Poverty %:", round(mapResults$povertyPercent, 1), '<br>')) %>% #pop-up Menu
  addLegend(position = "bottomleft", pal = myPal, values =  mapResults$match, 
...
```{r staticMapResults}
#Repeating first three steps from previous chunk since we have overwrote the object
mapResults = cbind(na.omit(multiClassData), predictedProbs) %>% 
   mutate(cluster_group = recode(cluster_group, C1 = '2nd.Wave', C2 = '1st.Wave', C3 = 'Other'))

mapResults$LargestProbCluster =  colnames(mapResults[, 14:16])[apply(mapResults[, 14:16], 1, which.max)] 

mapResults$match = ifelse(mapResults$cluster_group == mapResults$LargestProbCluster, 'Yes', 'No') %>% as.factor()

# Preparing Data for Static Map
mapResults %<>% select(key_numeric, match) 
colnames(mapResults) = c('region', 'value') #renaming columns to match required inputs for county_choropleth()

# Creating the Map Using ggplot options
choro = CountyChoropleth$new(mapResults) #creating the plot object
choro$ggplot_scale = scale_fill_brewer(name="Match", type = "qual", palette= 'Paired') # using colorbrewer colors
choro$render() + theme(legend.position = "bottom") # rendering the plot
```

### State Summaries

In the code chunk below, we compute the percentage of correctly matched results (i.e., cluster results were correctly prediced by the multinomial regression model) by state.

```{r stateSummaries}
#Repeating first three steps from previous chunks since we have overwrote the object
mapResults = cbind(na.omit(multiClassData), predictedProbs) %>% 
   mutate(cluster_group = recode(cluster_group, C1 = '2nd.Wave', C2 = '1st.Wave', C3 = 'Other'))
mapResults$LargestProbCluster =  colnames(mapResults[, 14:16])[apply(mapResults[, 14:16], 1, which.max)] 
...
```{r overallByCluster}
mapResults %>% group_by(cluster_group, LargestProbCluster) %>% summarise(n = n()) %>% 
...
- On a county-level, the time series of COVID-19 new daily cases can be clustered into one of three clusters: (a) `Cluster.3` contains 1363 counties, which have had a limited outbreak of COVID-19, (b) `Cluster.1` contains 1294 counties which have primarily experienced a second wave of COVID-19, and (c) `Cluster.2` contains 451 counties which have predominately experience a first wave of COVID-19. Note that our total number of counties are slightly less than the number of counties in the contiguous U.S. since we did not have complete data on the confirmed cases for those counties.  

- Using a multinomial regression model, we have shown/quantified the impact of the following factors: (a) county type (rural/underserved vs not), (b) population density, (c) percentage of the county' population voting Republican in the 2016 Presidential Elections, (d) the affiliation of the state's governor [mayor in case of the District of Columbia], (e) percentage of seniors in the county, (f) the number of ICU beds per 10,000 Residents within the county, (g) the CDC region's categorization of the state, and (h) percent of the county's resident population in poverty on the odds of being in Cluster 1 or Cluster 2 compared to the baseline of Cluster 3.
