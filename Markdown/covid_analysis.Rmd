...
pacman::p_load(tidyverse, magrittr, dataPreparation, recipes, doParallel, psych, skimr, VIM, reshape2, # for data analysis
               ...
               leaflet, choroplethr, choroplethrMaps, rgeos, # for mapping
...
USA = getData(country = "usa", level = 2)  %>% # Get US polygon data from raster
  subset(!NAME_1 %in% c('Alaska', 'Hawaii')) # filtering to contiguous USA

USA = gSimplify(USA, tol=0.1, topologyPreserve=TRUE) %>% # reducing its size
  SpatialPolygonsDataFrame(., data = USA@data) # converting it to polygons data
...
stargazer(finalModel, type = 'html', p.auto = FALSE, out="../Data/multi.html") # tabulating Final Model 
```
</center>
...
- On a county-level, the time series of COVID-19 new daily cases can be clustered into one of three clusters: (a) `Cluster.3` contains 1,476 counties, which have had a limited outbreak of COVID-19, (b) `Cluster.1` contains 1,214 counties which have primarily experienced a second wave of COVID-19, and (c) `Cluster.2` contains 418 counties which have predominately experience a first wave of COVID-19. Note that our total number of counties are slightly less than the number of counties in the contiguous U.S. since we did not have complete data on the confirmed cases for those counties.  
