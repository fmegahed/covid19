...
popDensity = log(multiClassDF.sorted$popDensity) %>% as.matrix() %>% as.vector()
...
Betacoe = spatial.base6[, (1:(15*number_of_clusters))] # based on current number of predictors
...
coeffTable %<>% as.data.frame() %>% 
  select(paste0(paste0("b", rep(seq(0, 14), number_of_clusters)), '[', 
                sort(rep(1:number_of_clusters, 15)),
                ']' )) %>% # reordering cols by name
  select_if(~ !any(is.na(.)))  # dropping NA cols (corresponding to Cluster 1)

tCoeffTable = t(coeffTable)

tCoeffTable = cbind(tCoeffTable[1:15,], tCoeffTable[16:30,]) %>% data.frame()
...
samples_p = spatial.base6[, -(1:(number_of_clusters*15))]
...
for (counter in 2:number_of_clusters) {
...
df %<>% select(-c(fips, # removed since it was only used in the spatial model 
                  PercentSeniors)) # was not significant
...
invisible(stargazer(finalModel, type = 'latex', p.auto = FALSE, out="../Data/multi.tex", 
                    single.row = TRUE, header = FALSE)) # saved in data
```
